...
/* Area
ALERT: This function returns doubled oriented area
*/
...
/* Orientation
Only need to check at one of extreme polygon vertices, so that colinear doesn't happen. The function checks lower-left point.
*/
...
/* Convex hull
Andrew's monotone chain algorithm for building right and left chain.
*/
...
int pointVsPolygon(const Point<F1>& point, const Polygon<F2>& poly) {
	...
	for (int i = 0; i < n; ++i) {
		if (point == poly[i]) return 0; // edge case: point == lower-left of polygon
		int j = next(i, n);
...
/* Extreme points
If we draw a line through the extreme point in the given direction, there are no polygon vertices to the right of it.
The direction can be fixed for all vertices, or it can be dependent on the vertex. Ex: direction from a given point to the vertex.
*/
...
/* Maximum distance (squared) between two vertices of the convex polygon
*/
